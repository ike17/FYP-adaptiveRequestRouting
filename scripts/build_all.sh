...
#
# Builds Docker images locally (on the ProDesk) and imports them into k3s on
# both nodes. k3s nodes do not share the host Docker daemon, so images must be
# imported via 'k3s ctr images import' on each node.
#
# ProDesk (CPU node, local):  docker save | sudo k3s ctr images import -
# Laptop  (GPU node, remote): docker save | ssh GPU_NODE sudo k3s ctr images import -
#
# Both images go to both nodes:
#   - generation-service:v1  — scheduled on specific nodes via nodeSelector, but
#                              the image must be present on the node it lands on.
#   - smart-gateway:v1       — has no nodeSelector, may land on either node.
#
# Usage:
#   GPU_NODE=192.168.0.xx ./scripts/build_all.sh
#
# Required env var:
#   GPU_NODE — hostname or IP of the laptop (GPU k3s agent)
#
# Optional:
#   SSH_USER — SSH user for the laptop (default: current user)
...
# Copy trained model into smart-gateway context if it exists
...
# ─── Import both images on both nodes ─────────────────────────────────────────
