...
def get_rag_url() -> str:
    """Get the RAG app URL from Minikube."""
    try:
        ...
        print(f"Warning: Could not get Minikube URL: {e}")
...
def send_query(url: str, prompt: str, timeout: int = DEFAULT_TIMEOUT) -> dict:
    """
    Send a single query to the RAG application.
...
def run_experiment(
    ...
    if successful:
        ...
        stats = {
            ...
            "std_latency_ms": (sum((x - stats.get("mean_latency_ms", 0))**2 for x in latencies) / len(latencies)) ** 0.5 if len(latencies) > 1 else 0,
            "timestamp": datetime.utcnow().isoformat()
        }
        # Recalculate std after mean is set
        mean = stats["mean_latency_ms"]
        stats["std_latency_ms"] = (sum((x - mean)**2 for x in latencies) / len(latencies)) ** 0.5
    else:
        stats = {
            "config": config_name,
            "total_queries": n_queries,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None,
        help="RAG app URL (auto-detected from Minikube if not specified)"
    )
    parser.add_argument(
        "--skip-wait", 
        action="store_true",
