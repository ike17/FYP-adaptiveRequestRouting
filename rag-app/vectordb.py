# vectordb.py
# in-memory vector store using chromadb for the retrieval stage of the rag pipeline
...
class VectorDB:
    ...
    def _initialize_knowledge_base(self):
        """load the sample documents that queries will be answered from"""
        documents = [
            # kubernetes concepts
    ...
    def retrieve(self, query: str, top_k: int = 3) -> list[str]:
        """semantic search - this is the cpu-bound part of the rag pipeline"""
        results = self.collection.query(
            query_texts=[query],
            n_results=top_k
        )

        # chromadb returns a nested list, we just want the strings
